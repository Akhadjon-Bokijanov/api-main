 ...
 * Site controller
 */
class ActController extends BaseApi
{
    ...
    public function actionUpdate($tin,$id){
        ...
        try {
            ...
            if($reason==""){
                $collection = Yii::$app->mongodb->getCollection('act');
                $collection->update(["ActId"=>$id],$data);
//                $id = get_object_vars($collection);
//                if(!isset($id['oid']))
//                    $reason = "Malumotlarni saqlashda xatolik";
            }
            if($reason==""){
                $modelDoc->SetActData($data);
                if(!$modelDoc->save()){
...
//                    var_dump($items);die;
                    if($i>=ExcelConst::ROW_BEGIN_KEY) {
                        $productsItesm[] =
                            [
                                "ProductName" => $items[ExcelConst::ACT_NAME],
                                "ProductMeasureId" => $items[ExcelConst::ACT_CODE],
                                "ProductCount" => (int)$items[ExcelConst::ACT_COUNT],
                                "ProductSumma" => round((float)$items[ExcelConst::ACT_PRICE], 2),
                                "ProductDeliverySum" => round((float)$items[ExcelConst::ACT_DELIVER_SUM], 2),
                                "ProductVatRate" => round((float)$items[ExcelConst::ACT_VAT_RATE], 2),
                                "ProductVatSum" => round((float)$items[ExcelConst::ACT_VAT_VALUE], 2),
                                "ProductDeliverySumWithVat" => round((float)$items[ExcelConst::ACT_DELIVER_WITH_RATE], 2),
                            ];
                    }
                }
//                echo $i;die;
    ...
    public function actionView($ActId,$tin){
        $reason = "";
        if(CompanyUsers::CheckCompany($tin)==false)
            $reason = "Kiritilgan STIR foydalanuvchiga tegishli emas.";
        if($reason==""){
            $query = new Query();
            $model =$query->from("act")
                ->andWhere(['ActId'=>$ActId])->limit(1)->all();
            return [
               'success'=>true,
               'data'=>$model
            ];
    ...
    public function actionIndex($tin){
        ...
        if($reason==""){
            ...
            $dataProvider->query->andWhere(['tin'=>$tin,'type'=>AllDocuments::TYPE_ACT]);
...
//    }
    ...
    protected static function VerifyBySign($data){
        ...
        if (isset($FacutraData['success']) && $FacutraData['success']==true) {
            ...
            $model = AllDocuments::findOne(['doc_id' => $FacutraData['ActId']]);
            if (!empty($model)) {
